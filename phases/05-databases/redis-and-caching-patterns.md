...
`caching-strategies` covers caching broadly; this topic is the database-side detail — Redis specifics, TTLs, eviction, and failure modes.
...
Three decisions follow, and they're the whole topic. *What goes in?* Only frequently used items, since the pantry is small and expensive. *How long do you trust it?* Milk has a date on it — that's a **TTL** (time to live), after which you go back to the supermarket rather than trusting old stock. *What happens when it's full?* You throw something out, and which item you discard is a policy choice called **eviction**.

The failure that matters: if the pantry empties at the exact moment fifty chefs need flour, all fifty run to the supermarket simultaneously. That's a **stampede**, and it can be worse than having no pantry at all.
...
**Cache-aside** (lazy loading) is the default and the one to name first. The application checks Redis; on a miss it reads the database, writes the result back with a TTL, and returns it. The cache fills on demand with only what's requested, and if Redis dies the application still works, just slower. Its weakness is that every miss costs a database read.

**Write-through** writes to cache and database together, keeping the cache fresh but paying both costs per write and caching data that may never be read. **Write-behind** writes to cache immediately and the database asynchronously — fastest, but if Redis dies before the flush those writes are gone.
...
**TTL strategy** matters more than people expect. A fixed TTL on keys written together means they expire together and stampede at once; adding **jitter**, a random spread of say 300 seconds ± 60, desynchronises expiry. The TTL *is* your maximum staleness, so pick it from how wrong the data may be.
...
Runs as-is on plain Node.js. The `cache` object has a Redis client's `get`/`set`/`del` shape, so swapping in the real one changes only those calls.
...
A homepage query is requested 5,000 times per second. Its cache entry expires, all 5,000 concurrent requests miss at once, all hit the database with the same expensive query, and the database — comfortable a moment ago — collapses. That's a **cache stampede**, and it's worse than no cache, since without one the database would have been sized for the real load.
...
Two related failures are worth naming. **Cache penetration** is repeated requests for a key absent from the database too, so every request reaches it — an attacker can exploit this deliberately, and the fix is caching the negative result. **Cache avalanche** is many keys expiring together, which jitter prevents.

The deepest trap is a cache that stops being optional. If your database only survives at a 95% hit rate, Redis is a load-bearing dependency and a restart takes production down cold. The honest test: *can the database survive a cold cache?*
...
| Write-through | Every write pays both | Always fresh | Caches unread data |
| Write-behind | Cheapest | Fresh in cache | Data loss if Redis dies |
